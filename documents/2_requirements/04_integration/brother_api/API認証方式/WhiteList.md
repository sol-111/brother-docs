...
| WAF地理制限 | 日本国内IPのみ許可。固定IP不要で海外からのアクセスを一括遮断 |
...
| 案 | メリット | デメリット |
|---|---|---|
| **案1: API Key＋標準機能**（PoC推奨） | ・追加費用・追加構築なしで即時運用可能（貴社側のWAF/API Gateway設定のみ）<br>・通信経路に第三者が入らない | ・送信元IPによる制限は行わないため、API Key漏洩時は地理制限・レートリミットの範囲でしか防げない（ローテーションで影響期間を限定） |
